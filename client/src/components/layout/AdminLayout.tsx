...
export function AdminLayout({ children }: AdminLayoutProps) {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setUserData({
            username: data.username || user.email?.split("@")[0] || "Admin",
          });
        }
      } catch (error) {
  ...
  const displayName = userData?.username || user?.email?.split("@")[0] || "Admin";
  ...
  const sidebarLinks = [
    ...
    { href: "/admin/tutor-inquiries", icon: Building2, label: "Tutor Inquiries" },
  ...
  return (
    <div className="min-h-screen bg-muted/20 flex">
      ...
      <main className="flex-1 ml-64 flex flex-col">
        {/* Header */}
        <header className="h-16 bg-background/80 backdrop-blur-md border-b border-border sticky top-0 z-20 px-6 flex items-center justify-end">
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                  <div className="text-right hidden sm:block">
                    ...
                    <p className="text-xs text-muted-foreground">PrepMaster Admin</p>
                  </div>
